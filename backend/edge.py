...
class Edge(Browser):
    ...
    #to get last visit_time from edge log file
    def get_visit_time_from_file(self):
        #edge log file path
        edge_log_file = os.path.expanduser('~\Documents\ezPass\Logs\edge_logs.txt')
        ...
        visit_time = None
        #check if edge log file is available
        #get visit_time value from log file
        if(os.path.isfile(edge_log_file)):
            with open('edge_logs.txt', "r") as f:
                for line in f:
    ...
    #write the latest visit_time to edge log file
    def write_visit_time(self):
        ...
        if(check_running_process('msedge')):
            ...
            visit_time = rows[0][0]
            conn.commit()
            conn.close()        #close edge history db
            ...
            with open('edge_logs.txt', "w") as f:
                f.write(str1)
    ...
    def get_keywords_from_file(self):
        #edge keyword file path
        edge_keyword_file = os.path.expanduser('~\Documents\ezPass\Keywords\edge_keywords.txt')
        ...
        past_keywords = []
        #check if edge keywords file is available
        #get keywords list from keywords file
        if(os.path.isfile(edge_keyword_file)):
            with open('edge_keywords.txt', "r") as f:
    ...
    def write_keyword_freq_to_file(self, sorted_dict):
        ...
        with open('edge_keywords.txt', "w") as f:
            f.write(str1)
    ...
    def generate_keywords(self):
        ...
        #update edge keywords file with updated keywords and freq list
